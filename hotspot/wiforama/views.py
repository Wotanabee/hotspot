...
def manage_admin_image(request):
    ...
    target_file = str(settings.SHARE_ROOT + image)
...
def manage_import_image(filename):
    print("#[manage_import_image] - Start Function")
    new_file = {}
    ...
    if filetype.is_image(new_name):
        print(f"#[manage_import_image] - {new_name} is a valid image")
        ...
        print(f"#[manage_import_image] - {file} is a valid video")
    
    print("#[manage_import_image] - Stop Function")          
    
    return new_file

def create_thumbnail(name_file):
    print(f"#[create_thumbnail] - Start Function")
    ## Definition de la taille du thumbnail
    size = (200, 200)
    print(f"#[create_thumbnail] - Size 200x200")
    target_folder = settings.MEDIA_ROOT
    target_file = target_folder + 'thumbnail_' + name_file
    print(f"#[create_thumbnail] - target_file {target_file}")
               
    if not os.path.exists(target_file):
        print(f"#[create_thumbnail] - Create thumbnail")
        ## Ouverture de l'image
        thumbnail = Image.open(target_folder + name_file)
        thumbnail.thumbnail(size)
        thumbnail.save(target_file)   

    print(f"#[create_thumbnail] - Stop Function")
