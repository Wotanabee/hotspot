...
#    target_file = str('/home/pi/upload/' + image)
    target_file = str('/home/pi/hotspot_project/media/' + image)
        
    if action == 'valid':
#        print('##### Validation de la photo : ', image)
        if os.path.isfile(path_file):
#            print('##### Source file : ', path_file)
#            print('##### Source target : ', target_file)
#            ssh = SSHClient()
#            ssh.set_missing_host_key_policy(AutoAddPolicy())
#            ssh.load_host_keys(os.path.expanduser(os.path.join("~", ".ssh", "know_hosts")))
#            ssh.connect('192.168.100.124', username='adm_franck', password='Qu1ckSh0w!')
#            sftp = ssh.open_sftp()
#            sftp.put(path_file, target_file)
#            sftp.close()
#            ssh.close()            
